...
| `fm-status-append.sh`    | Guarded status-line append a mode=no-mistakes ship brief routes its status report through; refuses a `done:` line until the pipeline has recorded a validated PR |
