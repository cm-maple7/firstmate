...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step attributed to that crew's branch, or an exact busy verdict from the semantic busy-state contract.
...
The one exception is a live run found through the coarse run-listing fallback, which is attributed on branch alone because a running pipeline rewrites the very tip it is validating.
The script header owns the exact run-head ancestry rules, and `nm_runs_status_for_branch` owns that fallback's selection order.
