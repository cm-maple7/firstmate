...
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh)'s header owns the exact branch, head, and pipeline-custody rules the rich `axi status` path applies, including the live pipeline-owned run that binds without head equality.
`nm_runs_status_for_branch` in the script owns the coarse run-listing fallback's selection order, where the branch's newest row decides and a live run for the branch outranks the head test because a running pipeline rewrites the very tip it is validating.
