...
# wait_for_exit calls here take the standard budget; wait_for_exit in
# tests/wake-helpers.sh owns what that budget is and why a tighter one reports
# spurious failures. The few away-mode cases that need a larger one pass it
# explicitly.
wait_numeric_file() {
...
test_turn_ended_not_working_surfaced() {
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a turn-end whose crew is not provably working"
...
test_turn_ended_still_pane_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a bare turn-end from an unchanged pane"
...
test_turn_ended_malformed_prior_hash_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a turn-end backed by a malformed prior hash"
...
test_turn_ended_trailing_newline_prior_hash_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a turn-end backed by a newline-terminated prior hash"
...
test_secondmate_turn_ended_churning_pane_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a churning secondmate turn-end"
...
test_turn_ended_colliding_window_key_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a turn-end with an ambiguous pane marker"
...
test_turn_ended_duplicate_endpoint_records_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a turn-end shared by two endpoint records"
...
test_turn_ended_mixed_positive_evidence_batch_default_off() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOWS="$(printf 'fm-firstoff\nfm-secondoff')" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a mixed-evidence batch without the opt-in flag"
...
test_status_and_turn_end_batch_never_uses_churn_evidence() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOWS="$(printf 'fm-firststatus\nfm-secondturn')" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a status-and-turn-end batch on churn evidence"
...
test_turn_ended_churn_absorb_off_by_default() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a churning turn-end without the opt-in flag"
...
test_turn_ended_churn_timer_write_failure_surfaced() {
  ...
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a churning turn-end without recording its deadline"
...
test_turn_ended_invalid_churn_bound_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" 2>/dev/null &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a turn-end with an invalid churn bound"
...
test_turn_ended_oversized_churn_bound_surfaced() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" 2>/dev/null &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a turn-end with an oversized churn bound"
...
test_working_note_not_working_surfaced() {
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not surface a working: note whose crew has no running pipeline and an idle pane"
...
test_secondmate_status_note_surfaced_despite_busy_agent() {
  ...
  pid=$!
  wait_for_exit "$pid" || fail "watcher absorbed a busy secondmate's routed status note"
...
test_self_announced_close_does_not_rewake_but_next_note_does() {
  ...
  wait_for_exit "$pid" || fail "a later different note after a self-announced close was swallowed"
...
test_unreadable_status_reports_once_per_file_state() {
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || { reap "$pid"; fail "a dangling status symlink was not reported"; }
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || { reap "$pid"; fail "a changed unreadable status did not report again"; }
  [ "$(status_presentation_marker_offset "$marker" "$status_file")" = 0 ] \
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || { reap "$pid"; fail "a readable replacement did not surface preserved content"; }
...
test_permission_recovery_surfaces_preserved_status() {
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || { reap "$pid"; chmod 600 "$status_file"; fail "an unreadable regular status was not reported"; }
  [ "$(status_presentation_marker_offset "$marker" "$status_file")" = 0 ] \
  ...
  wait_for_exit "$pid" || { reap "$pid"; fail "readability recovery did not surface preserved content"; }
  grep -Fx "signal: $status_file" "$out" >/dev/null \
...
#
# Regression for the 2026-08-31 rd-tree-goal-replace-anchor incidents: a
# background no-mistakes validation can legitimately sit on an idle pane far
# longer than STALE_ESCALATE_SECS. wedge_timer_check's reverify_working path
# re-confirms the run-step still reads working at each threshold crossing
# instead of blindly time-escalating, so this stays absorbed for as long as
# the run genuinely is; only once the run-step stops reporting working
# (Phase C) does the ladder still escalate.
test_stale_terminal_status_overridden_by_active_run() {
  ...
  # Phase B: backdate the idle timer past the threshold while the run-step
  # still reads working - the reverify check re-confirms it and resets the
  # timer instead of escalating, no matter how long the background run has
  # already sat idle.
  ...
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
    reap "$pid"; fail "watcher exited past the threshold while still provably working (should re-verify and absorb): $(cat "$out")"
  fi
  [ ! -s "$out" ] || fail "a re-verified still-working stale printed a wake reason instead of absorbing"
  [ ! -s "$state/.wake-queue" ] || fail "a re-verified still-working stale enqueued a wake instead of absorbing"
  [ ! -s "$state/.wedge-escalations-$key" ] || fail "a re-verified still-working stale incremented the escalation count"
  [ -s "$state/.stale-since-$key" ] || fail "the timer was cleared instead of reset on re-verified absorb"
  [ "$(cat "$state/.stale-since-$key")" -gt "$(( $(date +%s) - 500 ))" ] || fail "the timer was not actually reset on re-verified absorb"
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional phase-B watcher stop"

  # Phase C: the run-step stops reporting working (it parked, needing a
  # captain decision) - backdating the timer again now escalates, exactly as
  # a genuinely wedged run always did.
  export FM_FAKE_CREW_STATE='state: parked · source: run-step · parked at gate: 1 finding(s)'
  echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  : > "$out"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" FM_STALE_ESCALATE_SECS=240 FM_POLL=1 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not escalate once the run-step stopped reporting working"
  grep -F "stale: $window" "$out" >/dev/null || fail "escalation did not print a stale wake"
  grep -F "possible wedge" "$out" >/dev/null || fail "escalation did not flag a possible wedge"
  unset FM_FAKE_CREW_STATE
  pass "a stale terminal-looking status is overridden and absorbed while a run is actively working, re-verified past every threshold instead of blindly wedge-escalating, then escalated once the run-step stops reporting working"
}

# --- non-terminal stale, crew provably working: absorbed, then re-verified --
# indefinitely, then escalated once it actually stops working ----------------
# A provably-working crew (an actively-running pipeline) legitimately sits on a
# static pane (e.g. waiting on CI) far longer than STALE_ESCALATE_SECS, so a
# non-terminal stale is absorbed and re-verified at every threshold crossing
# rather than blindly wedge-escalated on elapsed time alone; only once the
# run-step stops reporting working does the wedge timer escalate.
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
  ...
  # Phase C: the run-step stops reporting working (checks failed and the
  # pipeline is fixing them) - a genuinely different signature from a quiet
  # background wait, so backdating the timer again now escalates.
  export FM_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available'
  echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  : > "$out"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
  ...
  unset FM_FAKE_CREW_STATE
  pass "provably-working non-terminal stale is absorbed on first sight, re-verified past every threshold instead of blindly wedge-escalating, then escalated once the run-step stops reporting working"
}
...
test_exited_declared_pause_is_bounded_but_live_gate_surfaces() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "captain-held dead-agent pane did not re-surface on the bounded cadence"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "live external-decision gate did not surface immediately"
...
test_secondmate_paused_resurfaces_in_normal_mode() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not re-surface a paused secondmate"
...
test_secondmate_captain_held_resurfaces_in_normal_mode() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "watcher did not re-surface a captain-held secondmate"
...
test_paused_authoritative_working_preserves_wedge_timer() {
  ...
  # Still genuinely working past the threshold: reverify_working re-confirms
  # it and resets the timer instead of escalating, exactly as a plain
  # provably-working non-terminal stale now does.
  echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  : > "$out"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" FM_STALE_ESCALATE_SECS=240 FM_POLL=1 FM_SIGNAL_GRACE=1 \
  ...
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
    reap "$pid"; fail "authoritative working state escalated instead of re-verifying past the threshold: $(cat "$out")"
  fi
  [ ! -s "$out" ] || fail "a re-verified authoritative-working stale printed a wake reason instead of absorbing"
  [ -s "$state/.stale-since-$key" ] || fail "the timer was cleared instead of reset on re-verified absorb"
  [ "$(cat "$state/.stale-since-$key")" -gt "$(( $(date +%s) - 500 ))" ] || fail "the timer was not actually reset on re-verified absorb"
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional re-verify watcher stop"

  # The declared pause clears and the run-step stops reporting working: the
  # ladder still escalates once there is nothing left to re-verify. (The log
  # must also stop declaring paused: here - as long as it still does,
  # pause_state_class's declared-wait branch keeps routing an inconclusive
  # crew-state read to the bounded pause recheck rather than the ladder,
  # exactly as it does for a live external-decision gate.)
  printf 'working: upstream landed, resuming\n' > "$state/paused-working.status"
  sig=$(seen_sig "$state/paused-working.status"); printf '%s' "$sig" > "$state/.seen-paused-working_status"
  export FM_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available'
  echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "authoritative working state did not wedge-escalate once it stopped reporting working"
  ...
  unset FM_FAKE_CREW_STATE
  pass "a paused status overridden by authoritative working preserves its wedge timer, keeps re-verifying instead of escalating while genuinely working, then escalates once it stops"
}
...
test_wedge_escalation_marks_demand_deep_inspection_after_threshold() {
  ...
  # The run-step stops reporting working (the run itself has actually stopped,
  # not merely gone quiet) - reverify_working's re-check now finds nothing to
  # absorb on, so each round genuinely escalates rather than resetting.
  export FM_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available'
...
test_busy_pane_stable_hash_escalates_past_turn_age_bound() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "a stable-hash busy pane did not wedge-escalate past the turn-age bound"
...
test_busy_pane_changing_hash_escalates_past_turn_age_bound() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "a changing-hash busy pane did not wedge-escalate past the turn-age bound"
...
test_busy_pane_repeated_escalation_reaches_demand_deep_inspection() {
  ...
  while [ "$n" -le 3 ]; do
    ...
    PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
      ...
      FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
    pid=$!
    wait_for_exit "$pid" || fail "busy turn-age escalation round $n did not escalate: $(cat "$out")"
...
test_busy_declared_pause_is_rechecked_not_wedge_escalated() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || { reap "$pid"; fail "a declared pause past the long cadence was never rechecked"; }
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || { reap "$pid"; fail "a lifted pause on an over-age busy pane no longer wedge-escalates"; }
...
# watcher, the default wait_for_exit budget for an escalating one), because the
# poll these tests assert on is the ONE poll that spawns the bounded worktree
# walk: on a loaded runner it outlives a fixed liveness budget, and a round
# reaped before it finished reports a lost deferral instead of the deferral
# under test.
test_wedge_escalation_deferred_while_worktree_is_written() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "a stalled crew that wrote nothing did not wedge-escalate on the existing schedule"
...
test_write_deferral_resurfaces_on_the_bounded_cadence() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "a long-running write deferral never re-surfaced on the bounded cadence"
...
test_secondmate_home_supervision_churn_is_not_write_evidence() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "a mate home's own supervision churn deferred an escalation it must not defer"
...
test_terminal_first_sight_drops_a_finished_write_deferral_chain() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "a first-sight captain-relevant status was not surfaced"
...
test_procevent_unacknowledged_result_redrains_until_handled() {
  ...
  procevent_watch_bg "$dir" "$out"
  pid=$!
  wait_for_exit "$pid" || fail "the first proactive wake never happened: $(cat "$out")"
...
test_procevent_marker_keys_are_injective() {
  ...
  procevent_watch_bg "$dir" "$out"
  pid=$!
  wait_for_exit "$pid" || fail "colliding-looking process-event keys were not surfaced"
...
test_procevent_surface_crash_boundaries() {
  ...
  wait_for_exit "$pid" || fail "the record was not replayable after output failure"
  ...
  wait_for_exit "$pid" || fail "a pre-marker crash was not replayable"
...
test_procevent_marker_failure_exits_and_replays() {
  ...
  pid=$!
  wait_for_exit "$pid" || fail "marker failure did not end the actionable watcher cycle successfully"
  ...
  pid=$!
  wait_for_exit "$pid" || fail "marker failure did not leave the durable record replayable"
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
  ...
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_POLL=1 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=1 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "heartbeat backstop did not surface an unsurfaced captain-relevant status"
  grep -Fx "heartbeat" "$out" >/dev/null || fail "backstop did not exit with a heartbeat wake"
  [ "$(status_presentation_marker_offset "$state/.hb-surfaced-miss" "$state/miss.status")" = \
    "$(size_of "$state/miss.status")" ] \
...
test_afk_signal_records_heartbeat_endpoint() {
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_for_exit "$pid" || fail "afk watcher did not hand the actionable signal to the daemon"
...
test_afk_paused_changed_pane_hands_off_plain_stale() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" || fail "AFK paused changed pane did not hand off a stale wake"
