...
#
# No current caller: fm-crew-state.sh's coarse runs scan stops at the branch's
# NEWEST row unconditionally, which subsumes this test and holds in the case
# this test cannot see. In a firstmate task worktree the lane head is normally
# resolvable anyway - worktrees share one object store with the primary
# checkout and the pipeline publishes lane heads into it as
# `refs/no-mistakes/sync/<run-id>` - so a resolvability guard reads a live
# run's advanced head as a proven mismatch. Weigh that before reintroducing it
# as an attribution gate; see nm_runs_status_for_branch for the evidence.
